...
namespace Pong
{
	...
	void AbstractLevel::_update_shader(Shader* shader)
	{
		...
		{
			shader->setVec3("pointLightPositions[" + std::to_string(i) + "]", _scene->get_point_light(i).position);
			shader->setVec3("pointLightColors[" + std::to_string(i) + "]", _scene->get_point_light(i).color);
		}
	}
	...
	void Level::run()
	{
		...
		Shader blinn_shader("blinn_v.glsl", "blinn_f.glsl");
	...
	*/
	...
	void PBRLevel::_level_setup()
	{
		...
		std::vector<Texture*> pbr_textures = { 
			_scene->create_texture("riron2_baseColor", "textures/rustediron2_basecolor.png", "albedoMap"),
			_scene->create_texture("riron2_metallic", "textures/rustediron2_metallic.png", "metallicMap"),
			_scene->create_texture("riron2_normal", "textures/rustediron2_normal.png", "normalMap"),
			_scene->create_texture("riron2_roughness", "textures/rustediron2_roughness.png", "roughnessMap"),
			_scene->create_texture("ao_map", "textures/aoMap.png", "aoMap")
		};
