...
namespace Pong {
    ...
    class CubeShape : public EditMesh {
    private:
        CubeSize _sizes{1.0, 1.0, 1.0};

        _P_NODISCARD std::vector<glm::vec3> _computeCubeVertices() const {
            std::vector<glm::vec3> tmpVert(8);

            // upper vertices
            tmpVert[0] = glm::vec3(_sizes.width / 2, _sizes.height / 2, _sizes.depth / 2);
            tmpVert[1] = glm::vec3(_sizes.width / 2, _sizes.height / 2, -_sizes.depth / 2);
            tmpVert[2] = glm::vec3(-_sizes.width / 2, _sizes.height / 2, -_sizes.depth / 2);
            tmpVert[3] = glm::vec3(-_sizes.width / 2, _sizes.height / 2, _sizes.depth / 2);

            // lower vertices
            tmpVert[4] = glm::vec3(_sizes.width / 2, -_sizes.height / 2, _sizes.depth / 2);
            tmpVert[5] = glm::vec3(_sizes.width / 2, -_sizes.height / 2, -_sizes.depth / 2);
            tmpVert[6] = glm::vec3(-_sizes.width / 2, -_sizes.height / 2, -_sizes.depth / 2);
            tmpVert[7] = glm::vec3(-_sizes.width / 2, -_sizes.height / 2, _sizes.depth / 2);
    ...
    public:
        ...
        _P_INLINE void set_size(CubeSize sizes) {
            _sizes = sizes;
        }

        _P_INLINE CubeSize &get_size() {
            return _sizes;
        }

        _P_INLINE CubeSize *get_size_ptr() {
            return &_sizes;
        }
    };
}
